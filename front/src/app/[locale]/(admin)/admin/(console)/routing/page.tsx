...
import type { ConfigValue, NodeTypeView, Page, ProviderStat } from '@/lib/api/admin-types';
...
export default async function AdminRoutingPage() {
  ...
  const [stats, routing, nodeTypes] = await Promise.all([
    adminFetch<Page<ProviderStat>>('/v1/admin/providers/stats'),
    // Weights are operator-only, so a reviewer sees the statistics without
    // the editor rather than an error page.
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <RoutingConsoleTabs nodeTypeCatalog={nodeTypes.items} providersPanel={providersPanel} />
    </div>
  );
}
